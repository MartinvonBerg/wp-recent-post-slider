...
namespace mvbplugins\recentpostslider;

defined('ABSPATH') or die('Are you ok?');

add_shortcode('recent-post-slider', '\mvbplugins\recentpostslider\show_recent_posts');
...
function show_recent_posts($attr)
{
	...
	$expires = (int) get_option( '_transient_timeout_recentpostslider_html_output', 0 ); // int value 0 if not set
	$transient_set_time = $expires - $transient_duration;

	if ( ($last_post_date > $transient_set_time) || \current_user_can( 'administrator' )) {
		\delete_transient( 'recentpostslider_html_output' );
	}

	// generate the output if not set in transient
	if ( false === ( $string = get_transient( 'recentpostslider_html_output' ) ) ) {
		...
		foreach ($custom_posts as $post) { 
				...
				if ( is_array($image_meta) ) {
					...
					*/
					...
					usort($image_sizes, '\mvbplugins\recentpostslider\cmp');
 ...
 *
 * @param array $a array1 for comparison
 * @param array $b array2 for comparison
 * @return integer result as integer: -1 or 1
 */
function cmp($a, $b) {
	if ($a['width'] == $b['width']) {
		return 0;
